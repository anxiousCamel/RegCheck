...
function generateResponsesTable(responses: Response[]): string {
  ...
  const rows = responses.map((r) => [r.statusCode.toString(), r.description]);
...
function generateRequestBodyExample(endpoint: ApiEndpoint): string {
  const examples: Record<string, Record<string, unknown>> = {
    ...
    saveFilledDataSchema: {
      fields: [{ fieldId: 'field-1', value: 'Valor preenchido' }],
    },
  };
 ...
 */
function generateResponseDataExample(endpoint: ApiEndpoint): Record<string, unknown> {
  // List endpoints
