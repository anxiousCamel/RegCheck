...
import { heading, table, list, mermaidDiagram } from '../markdown-formatter';
...
function generateModelSection(model: PrismaModel): string {
  ...
  if (relationFields.length > 0) {
    ...
    markdown += list(
      relationFields.map(
        (f) => `**${f.name}**: ${f.isArray ? 'Lista de' : 'Referência para'} \`${f.relationTo}\``,
      ),
      false,
    );
  }
 ...
 */
function getRelationshipDescription(rel: PrismaRelationship, _models: PrismaModel[]): string {
  const descriptions: Record<string, string> = {
