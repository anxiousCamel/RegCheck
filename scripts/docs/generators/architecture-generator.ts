...
import { heading, list, mermaidDiagram } from '../markdown-formatter';
...
`;
  ...
  markdown += list(
    ['Configuração Docker Compose para serviços locais', 'PostgreSQL, Redis, MinIO'],
    false,
  );
...
`;
  ...
  markdown += list(
    ['Schema Prisma com 10 modelos', 'Migrações de banco de dados', 'Cliente TypeScript type-safe'],
    false,
  );
