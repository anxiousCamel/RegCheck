...
import type { PrismaParserOutput, PrismaModel, PrismaField, PrismaRelationship } from './prisma-parser';
 ...
 */
export function generateERDCodeBlock(schema: PrismaSchema): string {
 ...
 */
export function generatePartialERD(schema: PrismaSchema, modelNames: string[]): string {
  // Filter models
  const filteredModels = schema.models.filter(m => modelNames.includes(m.name));
  ...
  const filteredRelationships = schema.relationships.filter(
    r => modelNames.includes(r.from) && modelNames.includes(r.to)
  );
  
  const partialSchema: PrismaSchema = {
