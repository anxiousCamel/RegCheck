...
console.log('📊 Models found:', schema.models.length);
schema.models.forEach(model => {
  ...
  if (relationFields.length > 0) {
    console.log(`    Relations: ${relationFields.map(f => `${f.name} -> ${f.relationTo}`).join(', ')}`);
  }
});

console.log('\n📋 Enums found:', schema.enums.length);
schema.enums.forEach(enumDef => {
  console.log(`  - ${enumDef.name}: ${enumDef.values.join(', ')}`);
});

console.log('\n🔗 Relationships found:', schema.relationships.length);
schema.relationships.forEach(rel => {
  console.log(`  - ${rel.from} ${rel.type} ${rel.to} (${rel.fromField}${rel.toField ? ` -> ${rel.toField}` : ''})`);
});
...
const expectedModels = [
  'PdfFile', 'Template', 'TemplateVersion', 'TemplateField', 
  'Document', 'FilledField', 'Loja', 'Setor', 'TipoEquipamento', 'Equipamento'
];

const expectedEnums = [
  'TemplateStatus', 'DocumentStatus', 'FieldType'
];

console.log('\n🧪 Verification:');
const foundModels = schema.models.map(m => m.name);
...
const foundEnums = schema.enums.map(e => e.name);
...
const templateModel = schema.models.find(m => m.name === 'Template');
if (templateModel) {
  console.log('\n  Template model:');
...
const equipamentoModel = schema.models.find(m => m.name === 'Equipamento');
if (equipamentoModel) {
  console.log('\n  Equipamento model:');
  console.log(`    - Fields: ${equipamentoModel.fields.length}`);
  const relations = getRelationFields(equipamentoModel);
  console.log(`    - Relations: ${relations.map(r => `${r.name} -> ${r.relationTo}`).join(', ')}`);
}
