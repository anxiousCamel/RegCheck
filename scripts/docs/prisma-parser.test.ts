...
`;
      ...
      const relationship = result.data.relationships.find(r => 
        (r.from === 'User' && r.to === 'Post') || (r.from === 'Post' && r.to === 'User')
      );
...
`;
      ...
      const templatePdfRelation = result.data.relationships.find(r => 
        (r.from === 'Template' && r.to === 'PdfFile') || (r.from === 'PdfFile' && r.to === 'Template')
      );
      expect(templatePdfRelation).toBeDefined();
      
      const templateFieldRelation = result.data.relationships.find(r => 
        (r.from === 'Template' && r.to === 'TemplateField') || (r.from === 'TemplateField' && r.to === 'Template')
      );
...
`;
      ...
      expect(model.fields.find(f => f.name === 'createdAt' && f.type === 'DateTime')).toBeDefined();
      expect(model.fields.find(f => f.name === 'updatedAt' && f.type === 'DateTime')).toBeDefined();
    });
