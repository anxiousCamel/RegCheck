...
export class EquipamentoService {
  ...
  static async list(page: number, pageSize: number, filters: EquipamentoFilterInput) {
    ...
    return cacheService.wrap(
      ...
      120,
    ); // 2 minutes TTL (more dynamic data)
  }
  ...
  static async create(input: CreateEquipamentoInput): Promise<EquipamentoDTO> {
    ...
    if (!loja || !loja.ativo)
      throw new AppError(400, 'Loja não encontrada ou inativa', 'INVALID_LOJA');
    if (!setor || !setor.ativo)
      throw new AppError(400, 'Setor não encontrado ou inativo', 'INVALID_SETOR');
    if (!tipo || !tipo.ativo)
      throw new AppError(400, 'Tipo não encontrado ou inativo', 'INVALID_TIPO');
  ...
  static async update(id: string, input: UpdateEquipamentoInput): Promise<EquipamentoDTO> {
    ...
    const data: Prisma.EquipamentoUncheckedUpdateInput = {};
    if (input.tipoId !== undefined) data.tipoId = input.tipoId;
    if (input.lojaId !== undefined) data.lojaId = input.lojaId;
    if (input.setorId !== undefined) data.setorId = input.setorId;
    if (input.numeroEquipamento !== undefined) data.numeroEquipamento = input.numeroEquipamento;
    if (input.serie !== undefined) data.serie = input.serie;
    if (input.patrimonio !== undefined) data.patrimonio = input.patrimonio;
    if (input.modelo !== undefined) data.modelo = input.modelo;
    if (input.ip !== undefined) data.ip = input.ip;
    if (input.glpiId !== undefined) data.glpiId = input.glpiId;
