      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground font-medium">
            Gerencie seus modelos de documentos técnicos
          </p>
        </div>
        <Button
      ...
      {isLoading ? (
        ...
        <div className="grid gap-4">
          {(
            ...
            <div
              key={template.id}
              className="flex flex-col sm:flex-row sm:items-center justify-between p-5 border-2 rounded-2xl bg-card hover:border-primary/30 transition-all shadow-sm gap-4"
            >
              <div className="space-y-2 flex-1">
                <div className="flex items-center flex-wrap gap-2">
                  <Link
                    href={`/editor/${template.id}`}
                    className="text-lg font-black hover:text-primary transition-colors uppercase tracking-tight"
                  >
                    {template.name}
                  </Link>
                  <Badge
                  ...
                  <Badge
                    variant="outline"
                    className="font-bold text-[10px] border-primary/20 text-primary bg-primary/5"
                  >
                    v{template.version}
                  </Badge>
                </div>
                ...
                <div className="flex items-center gap-3 text-[11px] font-bold text-muted-foreground/70 uppercase">
                  <span className="flex items-center gap-1">
                    <FilePlus className="h-3 w-3" /> {template.pageCount} páginas
                  </span>
            ...
            <div className="flex flex-col items-center justify-center py-20 text-center space-y-4 border-2 border-dashed rounded-3xl bg-muted/20">
              ...
              <div className="space-y-1">
                ...
                <p className="text-sm text-muted-foreground">
                  Comece importando um arquivo PDF base.
                </p>
              </div>
              <Button
                onClick={() => setShowUpload(true)}
    ...
    <div className="p-6 border-2 border-primary/20 rounded-3xl space-y-5 bg-primary/[0.02] animate-in fade-in slide-in-from-top-4 duration-300 shadow-xl">
      ...
      <div className="flex flex-col sm:flex-row gap-3 pt-2">
        <Button
          ...
          className="h-12 flex-1 font-bold gap-2"
        >
