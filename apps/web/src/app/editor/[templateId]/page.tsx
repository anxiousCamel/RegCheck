...
import { Save, Rocket, X, ChevronLeft, Layers, Settings2 } from 'lucide-react';
...
import type { FieldType, FieldScope, FillMode, FieldPosition, FieldConfig } from '@regcheck/shared';
...
export default function EditorPage() {
  ...
  useEffect(() => {
    ...
    // Safe cast: API returns template with fields array and pdfFile
    const t = template as { fields?: Array<{ id: string; type: string; pageIndex: number; position: unknown; config: unknown; scope?: string; slotIndex?: number | null; bindingKey?: string | null }>; pdfFile: { pageCount: number } };
  ...
  const templateData = template as
    | { name: string; status: string; fillMode: FillMode; pdfFile: { fileKey: string } }
    | undefined;
      ...
      <div className="flex items-center justify-between px-4 h-14 border-b border-slate-200 bg-white/80 backdrop-blur-md z-30">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button
            ...
            disabled={
              templateData?.status === 'PUBLISHED' || publishMutation.isPending || isLoading
            }
          ...
          >
            <div className="h-full flex flex-col p-4 overflow-hidden bg-white">
              ...
              <div className="flex-1 overflow-y-auto scrollbar-hide">
                <FieldProperties
                  templateId={templateId}
                  fillMode={templateData?.fillMode ?? 'AUTOMATICO'}
                />
              </div>
            </div>
          </div>
      ...
      <div className="h-8 bg-white border-t border-slate-200 flex items-center justify-between px-4 hidden sm:flex">
        <div className="flex items-center gap-4 text-[9px] font-bold text-slate-400 uppercase tracking-widest">
          <span className="flex items-center gap-1.5">
            <div className="w-1.5 h-1.5 rounded-full bg-green-500" /> Sistema Online
          </span>
