...
const items = [
  { href: '/cadastros/lojas', label: 'Lojas', description: 'Gerenciar lojas e filiais da rede', icon: Store, color: 'text-blue-500 bg-blue-50' },
  { href: '/cadastros/setores', label: 'Setores', description: 'Configurar setores e departamentos', icon: Map, color: 'text-indigo-500 bg-indigo-50' },
  { href: '/cadastros/tipos', label: 'Tipos', description: 'Definir categorias de hardware', icon: Box, color: 'text-amber-500 bg-amber-50' },
  { href: '/equipamentos', label: 'Equipamentos', description: 'Inventário completo de ativos', icon: Laptop, color: 'text-emerald-500 bg-emerald-50' },
];
  ...
  return (
    <div className="p-4 sm:p-8 space-y-10 bg-background/50 min-h-full">
      <div className="border-b pb-6">
        <h1 className="text-3xl sm:text-4xl font-black tracking-tighter uppercase">Painel de Cadastros</h1>
          ...
          return (
            <Link key={item.href} href={item.href} className="group">
              <div className="relative p-8 h-full border-2 rounded-[2rem] bg-card hover:border-primary/40 transition-all duration-300 shadow-sm hover:shadow-xl hover:-translate-y-1 flex flex-col items-start gap-6 overflow-hidden">
                ...
                <div className={cn("p-4 rounded-2xl transition-all duration-300 group-hover:scale-110", item.color)}>
