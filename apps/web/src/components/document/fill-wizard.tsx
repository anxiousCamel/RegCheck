        ...
        {groupedFields.map((field) => {
          ...
          if (field.type === 'checkbox') {
            return (
              <div
                key={field.id}
                className="bg-white p-4 rounded-xl shadow-sm border border-border flex flex-col gap-3"
              >
                <label className="text-[15px] font-bold text-foreground leading-tight">
                  {field.config.label}
                </label>
                <YesNo
                  value={
                    getValue(field.id, 0) === 'true' || getValue(field.id, 0) === true
                      ? true
                      : getValue(field.id, 0) === 'false' || getValue(field.id, 0) === false
                        ? false
                        : null
                  }
...
export function EquipmentStep({
  ...
  // Group fields
  const { groupedFields, hiddenFieldIds: _hiddenFieldIds } = groupDateFields(fields);

  // Type guard to narrow TemplateField from the union
  const isTemplateField = (f: DateGroup | TemplateField): f is TemplateField => f.type !== 'date-group';

  // Categorize fields
  const identificationFields = groupedFields.filter((f): f is TemplateField => isTemplateField(f) && f.type === 'text' && !!f.bindingKey);
  const checklistFields = groupedFields.filter((f): f is TemplateField => isTemplateField(f) && f.type === 'checkbox');
  const observationFields = groupedFields.filter((f): f is TemplateField => isTemplateField(f) && f.type === 'text' && !f.bindingKey);
  const photoFields = groupedFields.filter((f): f is TemplateField => isTemplateField(f) && f.type === 'image');
  const signatureFields = groupedFields.filter((f): f is TemplateField => isTemplateField(f) && f.type === 'signature');
  const dateGroups = groupedFields.filter((f): f is DateGroup => f.type === 'date-group');
  ...
  return (
    <div className="space-y-6 animate-in slide-in-from-right-4 duration-300">
        ...
        <section>
          ...
          <div className="bg-muted/30 p-4 rounded-xl border border-border/50 grid grid-cols-1 gap-y-3">
            {identificationFields
              ...
              .filter((f, i, self) => i === self.findIndex((t) => t.bindingKey === f.bindingKey))
              .map((f) => (
          ...
          <div className="space-y-3">
            {checklistFields.map((f) => (
              ...
              >
                ...
                <YesNo
                  value={
                    getValue(f.id, itemIndex) === 'true' || getValue(f.id, itemIndex) === true
                      ? true
                      : getValue(f.id, itemIndex) === 'false' || getValue(f.id, itemIndex) === false
                        ? false
                        : null
                  }
      ...
      <div className="flex gap-3 pt-4 border-t border-border mt-8">
        ...
        <Button onClick={onNext} className="h-14 flex-1 text-lg font-bold rounded-xl shadow-lg">
          {isLast ? 'Finalizar' : 'Próximo equipamento'}{' '}
      ...
      {(blobPreview || serverPreview) && (
        <>
          ...
          <button
            ...
            onClick={(e) => {
              e.stopPropagation();
              updateField(field.id, itemIndex, '');
            }}
          >
            <IconRefresh size={14} />
          </button>
...
interface DateGroup {
  ...
  monthIds: string[];
  yearIds: string[];
  config: { label: string; required?: boolean; placeholder?: string; [key: string]: unknown };
  bindingKey?: string;
}

function groupDateFields(fields: TemplateField[]) {
  const groupedFields: (DateGroup | TemplateField)[] = [];
  ...
  fields.forEach((f) => {
    ...
    if (suffix && ['dia', 'mes', 'mês', 'ano'].includes(suffix)) {
