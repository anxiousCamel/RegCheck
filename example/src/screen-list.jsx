    ...
    <div style={{background:'var(--rc-bg)', minHeight:'100%'}}>
      ...
      <PhoneHeader
        ...
        trailing={
          ...
          }}><IconMoreVertical size={18}/></button>
        }
      />
      ...
      <div style={{padding:'14px 14px 10px'}}>
        <div style={{
          ...
          <div style={{display:'flex', alignItems:'baseline', justifyContent:'space-between', marginBottom:10}}>
            <div>
              <div style={{fontSize:12, color:'var(--rc-fg-muted)', fontWeight:600, letterSpacing:'0.02em'}}>Progresso</div>
              <div style={{fontSize:22, fontWeight:800, letterSpacing:'-0.02em', marginTop:2}}>
                {done}<span style={{color:'var(--rc-fg-muted)', fontWeight:600}}>/{MOCK_DOC.totalItems}</span>
                <span style={{fontSize:13, color:'var(--rc-fg-muted)', fontWeight:500, marginLeft:6}}>equipamentos</span>
              </div>
            </div>
            <div style={{
      ...
      <div style={{
        ...
        <FilterChip active={setorId === null} label="Todos" count={MOCK_DOC.totalItems}
                    onClick={()=>setSetorId(null)}/>
        {MOCK_DOC.setores.map(s => (
          <FilterChip key={s.id} active={setorId === s.id} label={s.nome}
                      count={s.count} done={s.done}
...
function EquipmentRow({ a, onClick }) {
  const color = a.status === 'done' ? 'var(--rc-success)'
              : a.status === 'progress' ? 'var(--rc-primary)'
              : 'var(--rc-fg-subtle)';
  return (
    <button onClick={onClick} style={{
      ...
      <div style={{
        ...
        background: a.status === 'done' ? 'var(--rc-success-bg)'
                  : a.status === 'progress' ? 'var(--rc-primary-50)'
                  : 'var(--rc-bg-muted)',
        display:'flex', alignItems:'center', justifyContent:'center',
        flexShrink:0,
      }}>
      ...
      <div style={{flex:1, minWidth:0}}>
        <div style={{display:'flex', alignItems:'center', gap:8}}>
          <div style={{fontWeight:700, fontSize:14, letterSpacing:'-0.01em'}}>{a.numeroEquipamento}</div>
          <span style={{fontSize:12, color:'var(--rc-fg-muted)'}}>·</span>
          <div style={{fontSize:13, color:'var(--rc-fg-muted)'}}>{a.tipo}</div>
        </div>
        <div style={{fontSize:12, color:'var(--rc-fg-subtle)', marginTop:2,
