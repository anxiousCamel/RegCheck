  ...
  return (
    <div style={{background:'var(--rc-bg)', minHeight:'100%'}}>
      ...
      <div style={{padding:'14px 14px 110px', display:'flex', flexDirection:'column', gap:14}}>
        ...
        <div>
          <SectionLabel right={<Badge><IconSparkles size={10}/>Auto</Badge>}>Identificação</SectionLabel>
          <div style={{
        ...
        <div>
          ...
          <button onClick={onOpenSignature} style={{
            width:'100%', height:values.f_sig ? 110 : 72,
            borderRadius:12, border: values.f_sig ? '1px solid var(--rc-border)' : '1.5px dashed var(--rc-border-strong)',
            background: values.f_sig ? '#fff' : 'var(--rc-bg-muted)',
            cursor:'pointer', padding:12, position:'relative',
            display:'flex', alignItems:'center', justifyContent:'center', gap:10,
            color:'var(--rc-fg-muted)', fontSize:13, fontWeight:600,
          }}>
            {values.f_sig ? (
              <>
                {/* Mock signature stroke */}
                <svg width="180" height="60" viewBox="0 0 180 60" style={{color:'var(--rc-fg)'}}>
                  <path d="M10,40 C20,10 40,55 55,30 S85,20 95,35 S130,45 145,20 S170,35 175,25"
                        stroke="currentColor" strokeWidth="2" fill="none" strokeLinecap="round"/>
                </svg>
                <button style={{
                  position:'absolute', right:10, top:10,
                  width:28, height:28, borderRadius:8, border:'none',
                  background:'var(--rc-bg-muted)', cursor:'pointer',
      ...
      <div style={{
        ...
        <button className="rc-btn rc-btn--outline rc-btn--lg" style={{flex:'0 0 56px', padding:0}}
                onClick={onBack} aria-label="Anterior">
    ...
    <div style={{
      ...
      <div style={{flex:1, minWidth:0}}>
        <div style={{display:'flex', alignItems:'center', gap:6, flexWrap:'wrap'}}>
          <span style={{fontSize:14, fontWeight:600, letterSpacing:'-0.005em'}}>{field.config.label}</span>
          {field.config.required && (
            <span style={{fontSize:10, fontWeight:700, color:'var(--rc-danger)',
                          textTransform:'uppercase', letterSpacing:'0.04em'}}>Obrigatório</span>
          )}
        </div>
      </div>
  ...
  return (
    <button onClick={onClick} style={{
      ...
      <div style={{display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:6}}>
        ...
        {required && (
          <span style={{fontSize:10, fontWeight:700, color:'var(--rc-fg-muted)',
